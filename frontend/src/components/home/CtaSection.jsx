...
const CtaSection = ({ data: externalData }) => {
  ...
  return (
    <section className="py-14 sm:py-16 md:py-20 lg:py-24 bg-white border-t border-zinc-200">
      <div className="container mx-auto px-5 sm:px-6 md:px-8 max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-start">
          ...
          <div className="md:col-span-9 flex flex-col items-start fadeInRight">
            <h2 className="font-['Outfit'] text-[32px] sm:text-[40px] md:text-[52px] lg:text-[68px] font-bold text-zinc-900 leading-[1.15] lg:leading-[1.1] tracking-tight mb-8 sm:mb-10 lg:mb-12">
              {renderTitle(title)}
            </h2>
            
            {/* CTA Button*/}
            <Link to="/contact" className="max-w-full">
              <button className="group inline-flex items-center gap-3 sm:gap-4 rounded-full border border-zinc-300 hover:border-[#3B82F6] transition-colors pl-4 pr-2 sm:pl-6 py-2 opal-move-up cursor-pointer max-w-full">
                <span className="text-[11px] sm:text-[13px] font-bold tracking-wider text-zinc-600 uppercase group-hover:text-zinc-900 transition-colors truncate">
                  {buttonText}
                </span>
