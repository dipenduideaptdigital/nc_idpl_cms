      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-5xl h-[85vh] flex flex-col shadow-2xl dark:shadow-black/50 overflow-hidden font-sans transition-colors duration-300">
        ...
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50 dark:bg-zinc-800/50 shrink-0 transition-colors duration-300">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-zinc-800 dark:text-zinc-100 transition-colors duration-300">Select Media</h2>
            {/* Total File Count */}
            {meta && (
              <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 rounded text-xs font-bold border border-blue-200 dark:border-blue-500/20 transition-colors duration-300">
                {meta.total} Files
              </span>
            )}
          </div>
          <button onClick={onClose} className="p-2 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-zinc-50/50 dark:bg-zinc-950/50 transition-colors duration-300">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-zinc-400 dark:text-zinc-500 transition-colors duration-300">
              <p>No images found.</p>
            </div>
          ) : (
              ...
              {mediaList.map((media) => (
                ...
                >
                  ...
                  {selectedMedia?.id === media.id && (
                    <div className="absolute inset-0 bg-blue-600/20 dark:bg-blue-500/30 flex items-center justify-center">
            ...
            {meta?.totalPages > 1 && (
              <>
                <button
                  ...
                  className="p-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-md transition-colors disabled:opacity-50"
                >
