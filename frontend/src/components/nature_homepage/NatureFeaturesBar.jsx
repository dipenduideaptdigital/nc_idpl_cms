        ...
        {features.map((item, index) => {
          ...
          return (
            <div
              ...
              } ${index % 2 === 1 ? 'xs:border-l xs:border-white/10 lg:border-none' : ''}`}
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 2xl:w-14 2xl:h-14 flex-shrink-0 flex items-center justify-center filter brightness-125 contrast-125">
                <img src={iconUrl} alt={item.title} className="w-8 h-8 sm:w-10 sm:h-10 2xl:w-12 2xl:h-12 object-contain" />
              </div>
              <div className="flex flex-col text-left text-white">
                <span className={`font-kanit text-base sm:text-lg md:text-xl 2xl:text-2xl tracking-wide leading-snug ${item.titleBold ? 'font-bold text-white' : 'font-normal text-zinc-200'}`}>
                  {item.title}
                </span>
                <span className={`font-kanit text-xs sm:text-sm md:text-base lg:text-lg 2xl:text-xl tracking-wide leading-snug ${!item.titleBold ? 'font-bold text-white' : 'font-normal text-zinc-200'}`}>
                  {item.subtitle}
                </span>
              </div>
            </div>
