...
const BlogList = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'PUBLISHED': return <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Published</span>;
      case 'DRAFT': return <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-950/50 text-amber-700 dark:text-amber-400 uppercase tracking-wider">Draft</span>;
      case 'SCHEDULED': return <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 flex items-center gap-1 uppercase tracking-wider"><Clock className="w-3 h-3"/> Scheduled</span>;
      case 'ARCHIVED': return <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">Archived</span>;
      default: return <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">{status}</span>;
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-zinc-900 dark:border-emerald-400"></div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 flex items-center gap-2">
            <FileText className="w-6 h-6 text-zinc-900 dark:text-emerald-400" /> Blog Posts
          </h1>
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-xl flex items-center gap-3">
          <AlertCircle className="w-5 h-5 text-red-500 dark:text-red-400" /> <p>{error}</p>
        </div>
      )}

      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 overflow-hidden">
        <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between gap-4 bg-zinc-50/50 dark:bg-zinc-800/50">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2 border border-zinc-200 dark:border-zinc-700 rounded-xl leading-5 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-[#3B82F6] transition-colors sm:text-sm"
            />
          </div>
          ...
          <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            ...
            <tbody className="bg-white dark:bg-zinc-900 divide-y divide-zinc-100 dark:divide-zinc-800">
              {filteredBlogs.length > 0 ? filteredBlogs.map((blog) => (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      {blog.featuredImage ? (
                        ...
                        <div className="w-12 h-12 rounded-lg bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 dark:text-zinc-500"><FileText className="w-5 h-5"/></div>
                      )}
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          {blog.categories?.slice(0,2).map(c => (
                            <span key={c.id} className="text-[10px] bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded font-semibold">{c.name}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                ...
                <tr><td colSpan="4" className="text-center py-12 text-zinc-500 dark:text-zinc-400">No blogs found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
